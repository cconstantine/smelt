...
- **Every commit builds for both targets.** Before each commit, not only before calling a feature done, run the web check alongside the server tests:
  ```bash
