...
**2. One sandbox volume with a relative mount path breaks every new sandbox, and the failure reports as a timeout.** *Confirmed.*
The volume form accepted mount path `relative/path` (its own help text asks for a leading `/`). `create_volume` only expands `~` (`resolve_mount_path`) and stores it as-is, and it's mounted into every sandbox pod. Kubernetes requires an absolute `mountPath`, so every pod creation failed: the browser tier's `create_pod` reported `create_pod: Timeout` with no pod in the namespace at all. Deleting that one volume made the same test pass again.
- The "timeout" hides the real reason (the pod spec being rejected), which is why earlier `create_pod: Timeout` failures were misattributed to leftover pods.

Fix: reject a mount path that isn't absolute after `~` expansion, in `create_volume` (and the form); and make `create_pod` surface the API's rejection instead of waiting for a pod that was never created. Tests: `create_volume` refusing a relative path; `create_pod` returning the rejection promptly.
