...
`src/browser_tests.rs` is a small, `#[ignore]`d automated tier (real Postgres, real k3s, real headless Chrome via CDP) covering the sandbox panel and the context-usage indicator/detail view/compaction divider — not a general framework everything else is expected to plug into yet. For anything else that touches rendering or interaction, drive the running app manually with `dx serve --fullstack` and a browser, or a scripted headless Chrome session over the DevTools Protocol — `cargo check`/`cargo test` alone don't exercise hydration, click handlers, or the live SSE loop. See [testing.md](testing.md).
